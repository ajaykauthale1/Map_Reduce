 ...
 * @author Ajay
 *
 *         Fine Lock version that calculates the average of the TMAX
...
public class FineLockFib {
	 ...
	 * 
	 */
	public static void fineLockFibTMAX() {
		// load file and convert it into simple lines
		List<String> lines = LoaderRoutine.loadFile("1912.csv.gz");

		PrintStream out = null;
		try {
			out = new PrintStream(new FileOutputStream(LoaderRoutine.ROOT_DIR + "\\output\\finelockfib.txt"));
			...
			for (String line : unprocessedData) {
				...
				if (arr[2] != null && arr[2].equals("TMAX")) {
					...
					if (stationMap.containsKey(stationName)) {
						...
						// add temperature in synchronized so that station is locked
						newStation.addTempInSync(temp, true);
						stationMap.put(stationName, newStation);
					} // else add new station with TMAX
					else {
						newStation = new StationModel();
						// add temperature in synchronized so that station is locked
						newStation.addTempInSync(temp, false);
						stationMap.put(stationName, newStation);
					}
				}
			}
